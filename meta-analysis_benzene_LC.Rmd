...
**Any question? email [w.wan\@uu.nl](mailto:w.wan@uu.nl){.email} or
[charleswanwen\@gmail.com](mailto:charleswanwen@gmail.com){.email}**

##################### 

This script follows the systematic search of papers on the effect of
occupational benzene exposure on lung cancer up till 2024, and stands
for the formal meta-analysis (MA) of the risk estimates extracted from
these studies. Details for the data extracted and the screening process
are recorded in the associated paper. The main structure of the
following codes follows:

1.  for studies only reported exposure within several exposure
    categories without reporting ever-exposure effect, collapse risk
    estimates with fixed-effect MA
2.  ever exposure MA [main analysis, fig 2]
3.  do various sensitivity analyses: 3.1 Meta-regression over three
    studies that reported original benzene exposure scales (with ref to
    <https://ehp.niehs.nih.gov/doi/10.1289/ehp.1306880>) 3.2 break-up
    pooled studies from Wan et al. (2024) and include these as
    individual studies to be included with other studies for MA on
    ever-exposure 3.3 do various subgroup analyses 3.3.1 effect sizes
    from highest vs. lowest exposure category used 3.3.2 overall quality
    (low, moderate, high) 3.3.3 exposure assessment quality (low,
    medium, high) 3.3.4 whether smoking was adjusted 3.3.5 stratify by
    continent 3.4 combine all results from 3.3 to produce one figure
    (fig 3)\
    3.5 pub bias
...
#1. calculate the ever-exposure estimates from studies with multiple
exposure categories group that did not report
...
please note for Koh et al. (2011)\_///note: not to be pooled, estimates
of all workers will be used instead.
...
# Forest plot with updated labels

order_dat <- order_dat %>% 
  mutate(custom_study_labels = c(
"Wong et al. (1987)⁵¹", 
"Greenland et al. (1994)⁵⁶", 
"Collins et al. (2003)³⁶",
"Sorahan et al. (2005)⁵⁴", 
"Hoshuyama et al. (2006)⁵⁵", 
"McHugh et al. (2010)⁵⁷",
"Koh et al. (2011)⁵²", 
"Koh et al. (2014)⁵³",
"Linet et al. (2015)⁴⁸", 
...
# png(file = "Figure 3.png", width = 900, height = 600)
forest(ma_ever,
       xlim = c(0.8, 2), 
       pooled.totals = T,
       random = T,
       ...
       # studlab = custom_study_labels # Use custom labels
       )
# dev.off()
...
##3.1 Meta-regression over studies that reported original benzene
exposure scales

mid-point was used to except for the highest exp category, where 5/3 \*
...
##3.2 break-up pooled studies from Wan et al. (2024) and include these
as individual studies to be included with other studies for MA on
ever-exposure
...
3.3 do various subgroup analyses 3.3.0 study design (external, internal,
cohort, case-control) 3.3.1 effect sizes from highest vs. lowest
exposure category used 3.3.2 overall quality (low, moderate, high) 3.3.3
exposure assessment quality (low, medium, high) 3.3.4 whether smoking
was adjusted 3.3.5 stratify by continent 3.4 combine all results from
3.3 to produce one figure (fig 3)\
3.5 pub bias
...
###3.3.2-5 subgroup (batch producing) 3.3.2 overall quality (low,
moderate, high) 3.3.3 exposure assessment quality (low, medium, high)
3.3.4 whether smoking was adjusted 3.3.5 stratify by continent
...
pdf(file = "Figure 5.pdf", width = 15, height = 7)
ggplot(sub_result, aes(x = group, y = rr, color = sub, shape = sub)) +
  scale_y_continuous(trans = "log10", limits = c(0.24, 2.35), breaks = c(0.3, 0.5, 1, 1.5, 2)) +
...
rob_result <- tribble(
  ...
  mutate(
    study = case_when(
      study == "Wong et al. (1987)" ~ "Wong~et~al.~(1987)^51",
      study == "Collins et al. (2003)" ~ "Collins~et~al.~(2003)^36",
      study == "Sorahan et al. (2005)" ~ "Sorahan~et~al.~(2005)^54",
      study == "Hoshuyama et al. (2006)" ~ "Hoshuyama~et~al.~(2006)^55",
      study == "Koh et al. (2011)" ~ "Koh~et~al.~(2011)^52",
      study == "Koh et al. (2014)" ~ "Koh~et~al.~(2014)^53",
      study == "Linet et al. (2015)" ~ "Linet~et~al.~(2015)^48",
      study == "Collins et al. (2015)" ~ "Collins~et~al.~(2015)^43",
      study == "Babigumira et al. (2023)" ~ "Babigumira~et~al.~(2023)^7",
      study == "DeMoulin et al. (2024)" ~ "DeMoulin~et~al.~(2024)^5",
      study == "Greenland et al. (1994)*" ~ "Greenland~et~al.~(1994)^56",
      study == "McHugh et al. (2010)*" ~ "McHugh~et~al.~(2010)^57",
      study == "Wan et al. (2024)*" ~ "Wan~et~al.~(2024)^6",
      TRUE ~ study
    )) %>% 
  as.data.frame() %>% 
    mutate(
...
pdf(file = "ROB_plot.pdf", width = 7, height = 5)
ggplot(data_long, aes(x = Domain, y = study, fill = rob, pattern = rob)) +
  ...
  scale_pattern_manual(values = c("Low risk" = "stripe", "Some concern" = "crosshatch", "High risk" = "circle"),
                       name = "",
                       guide = "legend") +
  scale_y_discrete(labels = parse(text = levels(data_long$study))) +
  theme_minimal() +
  labs(
       x = "",
       y = "") +
