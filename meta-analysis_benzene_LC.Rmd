...
data_long <- rob_result %>%
  ...
  mutate(Domain = factor(Domain, levels = c( "Population inclusion", "Exposure assessment", "Outcome assessment", "Confounding", "Follow-up", "Statistical analysis"))) %>% 
  mutate(rob = factor(rob, 
                         levels = c("low", "some concern", "high"), 
                         labels = c("Low risk", "Some concern", "High risk"))) %>% 
  mutate(study = factor(study, levels = rob_result$study))
