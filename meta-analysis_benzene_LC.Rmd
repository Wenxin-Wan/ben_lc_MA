  ...
  mutate(sub = factor(sub, levels = c(
         ...
         group = case_when(
           ...
           sub == "Study quality" ~ factor(group, levels = c("High (n=4)", 
                                                             "Moderate (n=8)", 
                                                             "Low (n=1)")),
           sub == "Exposure quality" ~ factor(group, levels = c("High (n=5)", 
                                                                "Moderate (n=4)", 
                                                                "Low (n=4)")),
           sub == "Smoking adjustment" ~ factor(group, levels = c("Adjusted (n=4)", 
                                                                  "Not adjusted (n=9)")),
...
pdf(file = "subplot.pdf", width = 15, height = 6)
ggplot(sub_result, aes(x = group, y = rr, color = sub, shape = sub)) +
  ...
  scale_shape_manual(values = c(15, 16, 17, 18, 19, 20, 21)) +  # Assign distinct shapes
