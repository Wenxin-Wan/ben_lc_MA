...
sub_result <- tribble(~sub, ~group, ~rr, ~lower_ci, ~upper_ci,
                      ...
                      "Exposure quality", "Medium", 1.1, 0.86, 1.42,
  ...
  as.data.frame() 
 # mutate(
 #    group = factor(paste(sub, group, sep = " - "), levels = unique(paste(sub, group, sep = " - ")))
 #  )
...
ggplot(sub_result, aes(x = group, y = rr, color = sub, shape = sub)) +
  ...
  scale_shape_manual(values = c(16, 17, 15, 18, 19, 20)) +  # Assign distinct shapes
  theme_minimal() +  # Minimal theme for clarity
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  # Rotate x-axis labels
  labs( y = "Relative Risk (RR)", title = "Relative Risk by Subgroups with 95% CI") +
  theme(legend.title = element_blank(), legend.position = "") +
  geom_hline(yintercept = 1, linetype = "dashed", linewidth = 0.3) +
  # coord_flip() +
  facet_row(~sub, scales = "free", space = "free")  # Facets based on `sub`, free scales and space
